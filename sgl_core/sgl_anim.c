...
sgl_anim_t* sgl_anim_create(void* obj,float start_value,float end_value,uint32_t duration,void (*animi_obj_callback)(void*,float),float (*anim_tim_callbaclk)(float))
{
    ...
    {
        #if DEBUG_ENABLE
        log_printf("the anim have not obj-----sgl_anim_create\r\n");
        #endif
        return NULL;
    }
    ...
    {
        #if DEBUG_ENABLE
        log_printf("the anim have not animi_callback-----sgl_anim_create\r\n");
        #endif
        return NULL;
    }
    ...
    {
        #if DEBUG_ENABLE
        log_printf("anim malloc fail-----sgl_anim_create\r\n");
        #endif
        return NULL;
    }
    ...
    {
        #if DEBUG_ENABLE
        log_printf("the anim_tim_callback is not exist,use sgl inside anim_tim_callback-----sgl_anim_create\r\n");
        #endif
...
void sgl_anim_remove(sgl_anim_t* anim)
{
    ...
    {
        #if DEBUG_ENABLE
        log_printf("the anim is not exist-----sgl_anim_remove\r\n");
        #endif
        return;
    }
...
/* 动画函数时间戳 */
static uint32_t sgl_anim_tick_stampe(uint32_t pre_tick)
{
...
void sgl_anim_handle(void)
{
    ...
    uint32_t add_time = sgl_anim_tick_stampe(sgl_anim_stamp);
    sgl_anim_stamp = sgl_get_Timestamp();
    float p = 0;
    ...
    while(p_anim != NULL)
    {
        ...
        p = p_anim->anim_tim_callbaclk((float)p_anim->act_time / (float)p_anim->duration);//时间回调函数
    ...
    #if DEBUG_ENABLE
    log_printf("sgl_anim_sum :%d-----sgl_anim_handle\r\n",sgl_anim_sum);
    #endif
}
